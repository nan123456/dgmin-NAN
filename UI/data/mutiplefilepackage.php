...
//	 header('content-type:text/html;charset=utf-8');
	 function getFiles(){
	 	$i=0;
		$_FILES1=array($_FILES['myfile']);
		foreach($_FILES1 as $file){
	...
	function uploadFile($fileInfo,$allowExt=array('jpeg','jpg','png','gif','wbmp'),$maxfile=2097152,$uploadpath='upload',$flag=true){
    if($fileInfo ['error']>0){
  	//匹配错误信息;
  	echo '失败！！！';
...
//	 $maxfile=2097152;//2M=1024*1024*2;允许上传的最大值;
  ...
  if ($fileInfo ['size']>$maxfile){
     echo '上传文件过大';
     echo "<script language=javascript>alert('上传文件过大');history.back();</script>";
  }
